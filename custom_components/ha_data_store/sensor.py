...
class HelperSummarySensor(SensorEntity):
    """辅助元素统计传感器。

    把「辅助元素」下所有实体聚合为一个新实体：
      状态值        = 辅助元素个数
      状态属性      = 每个辅助元素的明细列表（entity_id/name/icon/source_type/source_entity_id）
    """

    _attr_has_entity_name = True
    _attr_translation_key = "helper_summary"
    _attr_icon = "mdi:puzzle"
    _attr_native_unit_of_measurement = "个"

    def __init__(self, hass, device_info):
        self._hass = hass
        self._attr_unique_id = f"{DOMAIN}_helper_summary"
        # 强制固定实体 id：sensor.ha_data_store_helper
        self.entity_id = "sensor.ha_data_store_helper"
        self._attr_device_info = device_info
        self._attr_native_value = 0
        self._attr_extra_state_attributes = {}

    def _load_data(self):
        items = self._hass.data.get(DOMAIN, {}).get("helper_entities", []) or []
        entities = []
        for it in items:
            inst_list = it.get("entities") or []
            if not inst_list:
                # 无实体实例兜底：用 item 自身字段
                ...
                entities.append({
                    "entity_id": eid,
                    "name": it.get("device_name") or eid,
                    "icon": "",
                    "source_type": it.get("source_type") or "",
                    "source_entity_id": it.get("source_entity_id") or "",
                })
                continue
            for ent in inst_list:
                eid = getattr(ent, "entity_id", None) or it.get("entity_id") or ""
                if not eid:
                    continue
                name = getattr(ent, "_attr_name", None) or it.get("device_name") or eid
                icon = getattr(ent, "_attr_icon", None) or ""
                entities.append({
                    "entity_id": eid,
                    "name": name,
                    "icon": icon,
...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    # 辅助元素统计传感器：强制固定实体 ID 为 sensor.ha_data_store_helper
    try:
        reg = er.async_get(hass)
        new_eid = "sensor.ha_data_store_helper"
        old_eid = reg.async_get_entity_id("sensor", DOMAIN, helper_summary_sensor.unique_id)
        if old_eid and old_eid != new_eid:
            try:
                reg.async_update_entity(old_eid, new_entity_id=new_eid)
            except Exception as e:
                _LOGGER.warning("[HDS] 辅助元素统计传感器实体重命名失败（%s → %s）: %s", old_eid, new_eid, e)
        reg.async_get_or_create(domain="sensor", platform=DOMAIN,
                                unique_id=helper_summary_sensor.unique_id,
                                suggested_object_id="ha_data_store_helper")
    except Exception as e:
